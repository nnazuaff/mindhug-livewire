<x-layouts.app title="Upgrade ke Plus - MindHug">


    <div class="max-w-5xl mx-auto px-4 py-10 sm:py-14">
        @php
            $plan = \App\Models\SubscriptionPlan::where('slug', 'plus-bulanan')->first();
        @endphp
        ...
        @if ($plan)
            <div class="max-w-md mx-auto">
                <div
                    ...
                    <p class="mt-2 text-xs text-[#8b6f5c]">Rp {{ number_format($plan->price / 30, 0, ',', '.') }} / hari
                    </p>
                    ...
                    @auth
                        @if (auth()->user()->role === 'plus' && auth()->user()->plus_expires_at && auth()->user()->plus_expires_at > now())
                            <div
                                class="rounded-2xl bg-emerald-50 border border-emerald-200 px-5 py-4 text-sm text-emerald-700">
                                🎉 Kamu sudah Plus sampai
                                {{ auth()->user()->plus_expires_at->setTimezone('Asia/Jakarta')->format('d M Y') }}.
                            </div>
                        @else
                            <a href="{{ route('upgrade.checkout', $plan) }}"
