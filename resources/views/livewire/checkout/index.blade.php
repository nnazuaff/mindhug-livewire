        ...
        @if ($pendingOrder)
            <div
                ...
                <a href="{{ route('orders.show', $pendingOrder->invoice_number) }}"
                    class="text-sm font-semibold text-[#a47551] hover:text-[#8f6243] whitespace-nowrap">Bayar →</a>
            </div>
        @endif
        ...
        <section class="rounded-[1.75rem] border border-stone-200/60 bg-white p-6 sm:p-8 shadow-sm">
            ...
            @if (empty($selectedAddress))
                <div class="mt-6 rounded-2xl border border-amber-200/80 bg-amber-50/70 p-5 text-sm text-[#8f5d34]">
                    ...
                    <p class="mt-1">Silakan tambahkan alamat utama di menu <a href="{{ route('account.addresses') }}"
                            class="font-semibold underline hover:text-[#a47551]">Alamat Saya</a>.</p>
                </div>
            @else
                <div class="mt-6 rounded-2xl border border-stone-200/60 bg-[#fefbf8] p-5">
                    ...
                    <div class="space-y-1 text-sm text-[#5f4a3f]">
                        ...
                        @if ($selectedAddress['detail'])
                            <p class="text-xs mt-1">Catatan: {{ $selectedAddress['detail'] }}</p>
                        @endif
                    </div>
                </div>
            @endif
