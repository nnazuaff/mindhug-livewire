<div wire:poll.10s="checkPaymentStatus" class="max-w-4xl mx-auto px-4 py-6 sm:py-10" x-data="{ snapToken: @entangle('snapToken') }"
    ...
    <div class="space-y-4 sm:space-y-6">
                ...
                @if (in_array($order->status, ['awaiting_payment', 'awaiting_confirmation', 'cancel_requested']) &&
                    ...
                    <div x-data="{ open: false }">
                        ...
                        <div x-show="open" x-cloak
                            ...
                            <div class="bg-white rounded-2xl w-full max-w-sm shadow-xl p-4 z-50">
                                <p class="text-sm font-semibold text-[#2b1d12] mb-2">Request Pembatalan</p>
                                <textarea wire:model="cancelReason" rows="2" placeholder="Tulis alasan pembatalan..."
                                    class="w-full rounded-xl border border-[#e0d0c0] px-3 py-2 text-sm focus:outline-none focus:border-rose-400"></textarea>
                                <div class="flex gap-2 mt-3">
                                    <button wire:click="requestCancel({{ $order->id }})" @click="open = false"
                                        class="flex-1 rounded-xl bg-rose-500 px-3 py-2.5 text-sm font-medium text-white hover:bg-rose-600">Kirim</button>
                                    <button @click="open = false; $wire.set('cancelReason', '')"
                                        class="flex-1 rounded-xl bg-stone-100 px-3 py-2.5 text-sm font-medium text-stone-600 hover:bg-stone-200">Batal</button>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        {{-- Items --}}
        <div class="rounded-2xl sm:rounded-3xl border border-[#e8d5c4] bg-white p-4 sm:p-6" x-data="{ open: true }">
                    ...
                    @foreach ($order->items as $item)
                        @if ($item->product_id)
                            <a href="{{ route('product.detail', $item->product_id) }}" wire:navigate
                                class="flex items-center gap-3 rounded-xl border border-[#ede0d4] bg-[#fff7ed] p-3 hover:shadow-sm transition">
                            @else
                                <div
                                    class="flex items-center gap-3 rounded-xl border border-[#ede0d4] bg-[#fff7ed] p-3">
                        @endif
                    ...
                    @foreach ($order->trackingEvents as $event)
                        <div class="relative flex gap-3 sm:gap-4 pb-5 sm:pb-6 last:pb-0">
                            ...
                            <div class="pt-1 sm:pt-2 min-w-0">
                                <p class="font-semibold text-[#2b1d12] text-xs sm:text-sm">{{ $event->title }}</p>
                                @if ($event->description)
                                    <p class="text-[0.65rem] sm:text-xs text-[#6a5a4f] mt-0.5">
                                        {{ $event->description }}</p>
                                @endif
                                <p class="text-[0.65rem] sm:text-xs text-[#aaa] mt-1">
                                    {{ $event->occurred_at->timezone('Asia/Jakarta')->format('d M Y, H:i') }} WIB</p>
                            </div>
                        </div>
                    @endforeach
                </div>
    ...
    <div class="rounded-2xl sm:rounded-3xl border border-[#e8d5c4] bg-white p-4 sm:p-6" x-data="{ open: true }">
        ...
        <div x-show="open" x-collapse>
            <div class="mt-4 space-y-2 text-sm">
                ...
                @endphp
                <div class="flex justify-between"><span class="text-[#6a5a4f]">Subtotal
                        ({{ $order->items->sum('qty') }} produk)</span><span
                        class="font-semibold text-[#2b1d12] tabular-nums">Rp
                        {{ number_format($subtotal, 0, ',', '.') }}</span></div>
                <div class="flex justify-between"><span class="text-[#6a5a4f]">Ongkos Kirim</span><span
                        class="font-semibold text-[#2b1d12] tabular-nums">Rp
                ...
                <div class="flex justify-between text-base font-semibold"><span>Total</span><span
                        class="text-[#a47551] tabular-nums">Rp
                        {{ number_format($order->total_amount, 0, ',', '.') }}</span></div>
                @if ($order->payment_type)
                    <div class="h-px bg-[#ede0d4]"></div>
                    <div class="flex justify-between text-xs text-[#8b6f5c]"><span>Metode</span><span
                            class="font-medium">{{ strtoupper($order->payment_type) }}</span></div>
                @endif
            </div>
        </div>
    </div>
