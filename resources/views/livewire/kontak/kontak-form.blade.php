...
<div class="page-enter">
    ...
    <div class="max-w-5xl mx-auto px-4 py-10 md:py-14 grid md:grid-cols-5 gap-10 md:gap-14 items-start">
        ...
        <div class="md:col-span-3">
            ...
            {{-- Success state --}}
            @if ($sent)
            <div class="rounded-3xl bg-emerald-50 border border-emerald-100 px-6 py-8 text-center"
                ...
                <div class="w-14 h-14 rounded-full bg-emerald-100 flex items-center justify-center mx-auto mb-4">
                    <svg class="h-7 w-7 text-emerald-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
            ...
            <form wire:submit.prevent="kirim" class="space-y-5">
                ...
                <div>
                    ...
                    <textarea id="k-pesan"
                              ...
                              class="w-full rounded-2xl border border-[#e0d0c0] bg-[#fdfaf7] px-4 py-3.5 text-sm placeholder-[#b0a090] outline-none transition-all duration-200 focus:border-[#a47551] focus:ring-4 focus:ring-[#a47551]/10 resize-none @error('pesan') border-red-300 bg-red-50/50 @enderror"
                    ></textarea>
                    @error('pesan')
                    <p class="mt-1.5 text-xs text-red-500">{{ $message }}</p>
                    @enderror
        ...
        <div class="md:col-span-2 space-y-3">
            ...
            <p class="text-xs font-medium tracking-widest uppercase text-[#aaa] mb-4">Cara lain untuk menghubungi kami</p>
            ...
            <a href="{{ $card['href'] }}"
               ...
               class="group flex items-center gap-4 rounded-2xl border border-[#ede0d4] bg-white px-5 py-4 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
                <div class="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 bg-[#f7ede3] group-hover:scale-105 transition-transform duration-200">
                    <svg class="h-5 w-5" style="color: {{ $card['color'] }}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round">
                        {!! $card['icon'] !!}
                    </svg>
                </div>
                <div class="min-w-0">
                    <p class="text-xs font-medium text-[#999] uppercase tracking-wider leading-none mb-1">{{ $card['label'] }}</p>
                    <p class="text-sm font-semibold text-[#2b2b2b] truncate">{{ $card['value'] }}</p>
                </div>
                <svg class="h-4 w-4 text-[#ccc] ml-auto flex-shrink-0 group-hover:text-[#a47551] group-hover:translate-x-0.5 transition-all duration-200" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            {{-- Note --}}
            <div class="rounded-2xl bg-[#f7ede3] border border-[#ede0d4] px-5 py-4 mt-2">
                <p class="text-xs text-[#7a6457] leading-relaxed">
                    <span class="font-semibold">Butuh bantuan segera?</span> Jika kamu atau seseorang yang kamu kenal dalam bahaya, segera hubungi layanan darurat atau hotline kesehatan jiwa di atas.
                </p>
            </div>
        </div>
