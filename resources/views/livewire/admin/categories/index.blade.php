                ...
                <tbody class="divide-y divide-stone-100">
                    @foreach ($categories as $cat)
                        <tr class="hover:bg-stone-50/50 transition-colors">
                            ...
                            <td class="px-5 py-3 text-stone-500 text-xs hidden md:table-cell truncate max-w-xs">
                                {{ $cat->description ?: '-' }}</td>
                            <td class="px-5 py-3">
                                <div class="flex items-center gap-2">
                                    ...
                                    <button
                                        onclick="Livewire.dispatch('openEditCategory', { categoryId: {{ $cat->id }} })"
            ...
            <div class="bg-white rounded-2xl w-full max-w-md shadow-xl">
                ...
                <div class="p-6 space-y-4">
                    <div class="grid grid-cols-2 gap-4 text-sm">
                        <div>
                            <p class="text-stone-400 text-xs">Slug</p>
                            <p class="font-medium text-stone-700">{{ $viewingCategory->slug }}</p>
                        </div>
                        <div>
                            <p class="text-stone-400 text-xs">Jumlah Produk</p>
                            <p class="font-medium text-stone-700">{{ $viewingCategory->products_count }}</p>
                        </div>
                    </div>
                    @if ($viewingCategory->description)
                        <div>
                            <p class="text-sm font-medium text-stone-700 mb-1">Deskripsi</p>
                            <p class="text-sm text-stone-600 leading-relaxed">{{ $viewingCategory->description }}</p>
                        </div>
                    @endif
                    <div class="grid grid-cols-2 gap-3 text-xs text-stone-500 border-t border-stone-100 pt-4">
                        <div>Dibuat: <span
                                class="text-stone-700">{{ $viewingCategory->created_at?->setTimezone('Asia/Jakarta')->format('d M Y, H:i') }}</span>
                        </div>
                        <div>Diperbarui: <span
                                class="text-stone-700">{{ $viewingCategory->updated_at?->setTimezone('Asia/Jakarta')->format('d M Y, H:i') }}</span>
                        </div>
                    </div>
                </div>
            </div>
