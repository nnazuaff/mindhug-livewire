        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40" wire:click.self="closeModal">
            <div class="bg-white rounded-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto shadow-xl">
                <div
                    ...
                    <h2 class="text-lg font-semibold text-stone-800">Edit Produk</h2>
                ...
                <form wire:submit.prevent="updateProduct" class="p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input wire:model="badge" type="text"
                    ...
                    @if (!empty($existingPhotos))
                        <div>
                            ...
                            <div class="grid grid-cols-4 gap-2">
                                @foreach ($existingPhotos as $file)
                    ...
                    @if (!empty($deletedPhotos))
                        <div>
                            <label class="block text-sm font-medium text-amber-700 mb-1.5">Foto yang akan dihapus (klik
                                untuk batalkan)</label>
                            <div class="grid grid-cols-4 gap-2">
                                @foreach ($deletedPhotos as $filename)
                                    <div class="relative group opacity-50 hover:opacity-100 transition-opacity cursor-pointer"
                                        wire:click="restoreDeletedPhoto('{{ $filename }}')"
                                        title="Klik untuk batalkan hapus">
                                        <img src="{{ asset('storage/products/' . $productId . '/' . $filename) }}"
                                            class="w-full aspect-square rounded-xl object-cover border border-amber-300">
                                        <div
